...
def get_das_uri(argv):
    if "--das-uri" in argv:
        das_uri_id = argv.index("--das-uri") + 1
        if das_uri_id < len(argv):
            das_uri = argv[das_uri_id]
...
def check_reference_genome(refGenome, UCSC_DAS_URI=UCSC_DAS_URI):
    refGenomeList = list_UCSC_reference_genomes(UCSC_DAS_URI=UCSC_DAS_URI)
    return refGenome in refGenomeList


def get_chromosome_size(chrom, refGenome, UCSC_DAS_URI=UCSC_DAS_URI):
    return get_segment_size_from_UCSC(refGenome, chrom, UCSC_DAS_URI=UCSC_DAS_URI)


def check_chromosome_size(chrom, chromSize, refGenome, UCSC_DAS_URI=UCSC_DAS_URI):
...
def check_sequence(region, sequence, refGenome, UCSC_DAS_URI=UCSC_DAS_URI):
    ...
    regionSequence = get_sequence_from_UCSC(region, refGenome, UCSC_DAS_URI)
    assert_msg = f"\nSequence: {sequence}"
